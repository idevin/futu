...
class FileAdder
{
    ...
    protected function processMediaItem(HasMedia $model, \App\Models\Media $media, self $fileAdder)
    {
        $this->guardAgainstDisallowedFileAdditions($media, $model);

        $this->checkGenerateResponsiveImages($media);

        $media->setAttribute('media_library_id', $model->id);
        ...
        if ($this->generateResponsiveImages && (new ImageGenerator())->canConvert($media)) {
            $generateResponsiveImagesJobClass = config('media-library.jobs.generate_responsive_images',
